  ...
  <div>
    <div class="b-b padder-v text-lg text-thin">
      <div class="padder">
        ...
        <router-link :to="{name: 'agency_detail'}">商家详情</router-link>
        <span class="text-xs wfc-icon-arrow-right-thin m-r-xs"></span>
        <span class="text-muted">编辑商家信息</span>
      </div>
    </div>
     <div class="wrapper padder-md" v-if="code === 200">

       <rv-form v-model="formData"></rv-form>
  ...
  methods: {
    onSubmit () {
      if (isJson(this.formData)) {
        this.loading = true
        this.axios.post(api.agency.edit, JSON.parse(this.formData))
        .then(response => {
          if (response.data.code === 200) {
            // 提交成功
    ...
    success () {
      this.$router.replace({ name: 'agency' })
    }
  }
}
</script>
