  ...
  <div>
    ...
    <div class="b-b text-lg wrapper search-boxs">
      <div class="search-form pull-left search-box">
        <span>内容搜索</span>
        <input type="text" class="form-input m-t-n-xs" placeholder="内容搜索" v-model="searchData.title">
      </div>
      <div class="search-form pull-left search-box">
        <span>发布时间</span>
        ...
        <table class="table" v-if="list">
          ...
          <tbody>
            <tr v-for="item in list">
              <td>
                <div class="text-lg txt-add" v-html="item.title">
                  {{item.title}}
                </div>
              ...
              <td>
                <router-link :to="{ name: 'issue-detail', params: { id: item.id}}">详情</router-link>
...
export default {
  ...
  methods: {
    ...
    removeYes () {
      this.axios.get(api.questions.issue_rem, {params: {id: this.removeInd}})
      .then(response => {
    ...
    initData () {
      this.axios.get(api.questions.issue_list, {params: this.searchData})
      .then(response => {
        if (response.data.code === 200) {
