  ...
  <div class="wrapper">
    <table class="table table-bordered">
      <tbody>
        ...
        <tr>
          <td class="w-md">提问时间</td>
          <td>{{data.publish_time}}</td>
        </tr>
        <tr>
          <td class="w-md">提问标题</td>
          <td v-show='!isShow'>{{data.title}}</td>
          <td v-show='isShow' class="txt"><textarea v-model="data.title"></textarea></td>
        </tr>
        <tr>
          <td class="w-md">提问内容</td>
          <td v-show='!isShow'>{{data.content}}</td>
          <td v-show='isShow' class="txt"><textarea v-model="data.content"></textarea></td>
        </tr>
        <tr>
          <td class="w-md">提问图片</td>
...
export default {
  ...
  methods: {
    doDelete () {
      this.axios.get(api.questions.issue_rem, {params: {id: this.$route.params.id}})
      .then(response => {
    ...
    getDetail () {
      this.axios.get(api.questions.issue_detail, {params: {id: this.$route.params.id}})
      .then(response => {
        if (response.data.code === 200) {
    ...
    saveBtn () {
      this.axios.post(api.questions.issue_edit, {id: this.$route.params.id, content: this.data.content, title: this.data.title})
      .then(response => {
        if (response.data.code === 200) {
          this.isShow = false
