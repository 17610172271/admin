...
export default {
  ...
  methods: {
    doDelete () {
      this.axios.get(api.feed.comment_rem, {params: {id: this.$route.params.id}})
      .then(response => {
    ...
    getDetail () {
      this.axios.get(api.feed.comment_detail, {params: {id: this.$route.params.id}})
      .then(response => {
        if (response.data.code === 200) {
          this.data = response.data.data[0]
          if (isJson(this.data.imgpath)) {
            this.data.imgpath = JSON.parse(this.data.imgpath)
          }
        }
      })
    },
    ...
    saveBtn () {
      this.axios.post(api.feed.comment_edit, {id: this.$route.params.id, content: this.data.content})
      .then(response => {
        if (response.data.code === 200) {
          this.isShow = false
