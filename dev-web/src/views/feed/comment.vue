        ...
        <table class="table" v-if="list">
          ...
          <tbody>
            <tr v-for="item in list">
              ...
              <td>
                <router-link :to="{ name: 'com-detail', params: { id: item.id}}">详情</router-link>
...
export default {
  ...
  methods: {
    ...
    removeYes () {
      this.axios.get(api.feed.comment_rem, {params: {id: this.removeInd}})
      .then(response => {
    ...
    initData () {
      this.axios.get(api.feed.comment_list, {params: this.searchData})
      .then(response => {
        if (response.data.code === 200) {
