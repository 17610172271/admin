        ...
        <table class="table" v-if="list">
          ...
          <tbody>
            <tr v-for="item in list">
              ...
              <td>
                <router-link :to="{ name: 'con-detail', params: { id: item.post_id}}">详情</router-link>
                <a class="remove-btn" @click="removeBtn(item.post_id)">删除</a>
              </td>
            </tr>
          </tbody>
        </table>
...
export default {
  ...
  methods: {
    ...
    removeYes () {
      this.axios.get(api.feed.content_rem, {params: {id: this.removeInd}})
      .then(response => {
    ...
    initData () {
      this.axios.get(api.feed.content_list, {params: this.searchData})
      .then(response => {
        if (response.data.code === 200) {
